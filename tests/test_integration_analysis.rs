...
fn test_analyze_claude_code_conversation() {
    ...
    assert!(result.is_ok(), "Should successfully analyze Claude Code conversation");
...
fn test_analyze_codex_conversation() {
    ...
    let result = analyze_jsonl_file(&example_file);
    assert!(result.is_ok(), "Should successfully analyze Codex conversation");
    ...
    if let Ok(analysis) = result {
        if let Some(records) = analysis.get("records").and_then(|v| v.as_array()) {
            if let Some(record) = records.first() {
                ...
                if let Some(conversation_usage) = record.get("conversationUsage").and_then(|v| v.as_object()) {
    ...
    if let Ok(analysis) = result {
        if let Some(records) = analysis.get("records").and_then(|v| v.as_array()) {
            if let Some(record) = records.first() {
                ...
                if let Some(write_details) = record.get("writeFileDetails").and_then(|v| v.as_array()) {
                    for write_detail in write_details {
                        if let Some(file_path) = write_detail.get("filePath").and_then(|v| v.as_str()) {
                ...
                if let Some(read_details) = record.get("readFileDetails").and_then(|v| v.as_array()) {
                    for read_detail in read_details {
                        if let Some(file_path) = read_detail.get("filePath").and_then(|v| v.as_str()) {
                            assert!(!file_path.is_empty());
                        }
                    }
                }
...
fn test_analyze_unique_files_count() {
    ...
    if let Ok(analysis) = result {
        if let Some(records) = analysis.get("records").and_then(|v| v.as_array()) {
            if let Some(record) = records.first() {
                // Total unique files should be reasonable
                if let Some(unique_files) = record.get("totalUniqueFiles").and_then(|v| v.as_u64()) {
                    assert!(
                        unique_files >= 0,
                        "Unique files count should not be negative"
                    );
                }
            }
        }

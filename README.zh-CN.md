...
**前提条件**: [Rust 工具链](https://rustup.rs/) 1.85 或更高版本
...
│  Copilot: 15,000 tokens/天  |  $0.18/天                                                                          │
│  总体: 179,090 tokens/天  |  $1.20/天                                                                            │
└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
...
- 💾 显示内存使用量
- 📊 汇总统计
- 📈 按提供者（Claude Code、Codex、Copilot、Gemini）的每日平均值
...
2. **比较版本**：比较当前版本与最新可用版本（使用 `--force` 时跳过）
...
`--force` 标志会跳过版本检查，**总是下载**最新版本：

```bash
# 强制重新安装最新版本（适用于二进制文件损坏时）
vct update --force
```
...
**只有在以下情况才会失败**：找不到您平台（操作系统/架构）的二进制文件

### 🎯 适用于所有安装方式

无论您是通过 **npm**、**pip**、**cargo** 或**手动安装**，`vct update` 都以相同的方式工作：
...
**为什么可以这样做？** 所有安装方式（npm/pip/cargo/manual）都使用**相同的预编译二进制文件**，这些文件都来自 GitHub releases。update 命令只是下载最新的二进制文件并替换您当前的安装。
...
- Claude Code、Codex、Copilot 和 Gemini 团队创建了出色的 AI 编程助手
- Rust 社区提供了优秀的工具

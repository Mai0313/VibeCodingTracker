...
> [!NOTE]
> Model 行会按 cost 升序排序，因此花费最高的 model 会紧邻在 `TOTAL` 行上方。该排序适用于交互式面板、`--table` 与 `--text` 三种输出；`--json` 也会保持相同顺序。
...
- `~/.gemini/tmp/<project_hash>/chats/*.{json,jsonl}`（Gemini CLI — 同时支持旧版 `.json` 单一对象导出与现行的 `.jsonl` event stream）

...
Every update check records the result into `config.toml`'s `[update]` section via `version_cache::record_version_check` → `config::record_update_check` (`latest_version` + `last_checked_at`, preserving `check_enabled` / `dismissed_version` and comments) as groundwork for a future auto-update prompt. `last_checked_at` is stamped with `now_rfc3339_utc_nanos()` (RFC3339 UTC nanoseconds, e.g. `2026-07-07T05:34:50.563606999Z`) — the same stamp the per-CLI version caches (`{claude,codex,copilot,cursor}_version.json`, written by `detect_cli_version`) use for their `last_checked_at` field. The four per-CLI version caches refresh once per UTC day. (These update-check fields used to live in a standalone `~/.vct/version.json`, now folded into `config.toml` on first load; see the Configuration section.)

### Configuration (`src/config/`)

`~/.vct/config.toml` holds user settings, loaded **once** at the top of `main()` (`config::load()`) before the subcommand dispatch. `load` is infallible (missing/malformed → `Config::default()`) and, on first run, writes a **commented template** and folds any legacy `~/.vct/version.json` into `[update]` before deleting it (`migrate_legacy_version`). Reads use serde (`toml_edit::de`); writes go through `toml_edit`'s `DocumentMut` (`save_merge_models` / `record_update_check`) so hand-added comments and unknown keys survive — the same "preserve what we don't own" idea as `update_json_file_in_place`. The atomic write primitive is `utils::write_string_atomic` (temp-file + fsync + rename, shared scaffolding with `write_json_atomic`). Every public fn has a `*_in(dir, …)` seam so tests inject a temp dir (no `HOME` mutation).

Sections: `[general].default_time_range` (feeds `resolve_time_range_with_default`; an explicit period flag always wins), `[usage].merge_models` (seeds the dashboard; the `m` toggle persists via `save_merge_models`; `--merge-providers` forces on), `[usage].show_quota_panels` (when `false`, the usage TUI skips `QuotaPresence::detect()` so no credentials are probed and no quota workers spawn), and `[update]` (folded version.json state; `check_enabled` is groundwork for a future auto-check — explicit `vct update` always runs). The `vct config` subcommand (`path` / `show` / `edit`) manages the file. `UsageConfig` / `UpdateConfig` implement `Default` **by hand** so a missing section still defaults the opt-out booleans (`show_quota_panels` / `check_enabled`) to `true`, not `bool::default()`.

The interactive TUIs scroll with the **arrow keys only** (`↑`/`↓`); the shared `handle_input` (`src/display/common/tui.rs`) plus `NavDelta` were trimmed to line moves — no `PageUp/PageDown`, `g/G`, `Home/End`, or `j/k`.
...
- Test layout follows [Rust Book ch11-03](https://doc.rust-lang.org/book/ch11-03-test-organization.html): unit tests inline in `src/<module>/*.rs` inside `#[cfg(test)] mod tests`; each file under `tests/` compiles to its own binary, and `tests/common/mod.rs` is the shared helper module (`TempHome`, `fixture`). The integration-test split is one-file-per-subsystem:
    ...
    - `tests/config.rs` — `config::*_in` first-run creation, legacy `version.json` migration, comment-preserving writes, and `resolve_time_range_with_default` precedence, all rooted at a `TempHome`'s `~/.vct`

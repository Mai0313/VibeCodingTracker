...
Both subcommands support four output modes (interactive TUI / static table / plain text / JSON) and four time-range filters (`--daily` / `--weekly` / `--monthly` / `--all`) when scanning all sessions. The interactive TUI is the batch default when no format flag is given. `vct analysis FILE` instead prints complete JSON by default, accepts `--json` / `--text` / `--table`, and rejects time-range flags.
...
- **Claude Code**: A successful file tool appears once as an assistant `tool_use` invocation and again as a tool result. Count the invocation once, then attach result details without incrementing it again. `TaskCreate` and `TaskUpdate` map to the existing `TodoWrite` metric.
...
Global singleton `GLOBAL_FILE_CACHE` (capacity = 5 in `constants::capacity::FILE_CACHE_SIZE`) keyed by `PathBuf`, holding `Arc<CodeAnalysis>` — **typed** form, not `serde_json::Value`, because `to_value` deep-clones every string. Invalidation uses file metadata fingerprints; Grok entries also track `summary.json`, `updates.jsonl`, and the workspace `.cwd` marker. TUI keeps cache size small to bound RSS; bump deliberately if you change the displayed-sessions horizon.

...
//! `refresh_interval`, moving `[usage].quota_panels` into the nested
...
//! same pass). A read-time `migrate_legacy` shim then backstops any residual
//! legacy form the structural pass leaves alone (e.g. a hand-edited inline
//! `usage = { ... }` table), so the returned [`Config`] is always correct even
//! when the file was not rewritten.
...
///
/// Two cases skip the panel list itself: an empty one (the band was turned off
/// on purpose, and the marker is still stamped so it stays off), and one living
/// inside an inline table, which this pass never rewrites and therefore never
/// marks as upgraded. A file whose `[general]` is an inline table is skipped
/// outright: the marker has nowhere to go, so nothing here could stay one-shot.

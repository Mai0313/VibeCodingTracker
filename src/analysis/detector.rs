...
mod tests {
    ...
    fn test_detect_gemini_rejects_legacy_single_object() {
        // Legacy single-object Gemini exports carrying an inline `messages`
        // array must not be mis-classified — they should fall through to the
        // default so the user sees a clear failure instead of an empty analysis.
        let data = vec![json!({
    ...
    fn test_detect_copilot_rejects_legacy_single_object() {
        ...
        // rather than mis-routing the file to the JSONL analyzer.
        let data = vec![json!({
            "sessionId": "test-session",
    ...
    fn test_detect_claude_code_past_long_preamble() {
        ...
        // number of metadata preamble records before the first
        // `parentUuid`-bearing line. A previous implementation capped the
        // scan at 5 records and silently mis-classified long-preamble
        // sessions as Codex.
